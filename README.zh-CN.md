...
AI 和用户共享同一个 pty 会话，像一个会打字的搭档和你共用一个终端。
...
```bash
cp .env.example .env
# 编辑 .env，填写 ANTHROPIC_API_KEY 和 OPENAI_BASE_URL
```
...
| `MODEL_NAME`             | 使用的模型名                   | `claude-sonnet-4-20250514`       |
...
这个项目用中文编写，但欢迎国际贡献者。主仓库的 [README.md](README.md) 为英文版本。
