...
WinkTerm can use **Build Remote Agent** as a pairing device: the paid
iOS/Android app spectates (and can inject into) this desktop session through the
free MIT `gbr-agent`. Phone and PC never open ports to each other.

WinkTerm's own PTY / MCP / HTTP agent API on **`:8000`** is unchanged. Do not
reuse `WINKTERM_AGENT_TOKEN` or the localhost handshake as a GBR pair.
...
Phone is spectator. Orchestration stays on WinkTerm (shared PTY, winkterm-mcp,
or a bot talking to the same Bot API).
